...
class controllerFaculty extends CI_Controller
{
    ...
    public function submitFile()
    {
        ...
        // Check if the user has a pending file
        $hasPending = $this->model_faculty->hasPendingSubmission($user_id);
        if ($hasPending) {
            echo json_encode([
                'success' => false,
                'message' => 'You cannot upload a new file while your previous submission is still pending.'
            ]);
            return;
        }
        ...
        $next_rank_label = $this->getNextRankLabel($rank);
        $next_rank_order = $this->getNextRankOrder($rank);
