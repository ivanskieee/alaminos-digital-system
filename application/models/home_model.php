...
class home_model extends CI_Model
{
    ...
    public function updateHiddenFields($user_id, $hidden_fields)
    {
        // Delete existing hidden fields
        $this->db->where('user_id', $user_id)->delete('hidden_fields');

        // Insert new hidden fields
        foreach ($hidden_fields as $field) {
            $this->db->insert('hidden_fields', [
                'user_id' => $user_id,
                'field_name' => $field
            ]);
        }
    ...
    public function getHiddenFields($user_id)
    {
        $this->db->select('field_name');
        $this->db->where('user_id', $user_id);
        $query = $this->db->get('hidden_fields');
        return array_column($query->result_array(), 'field_name');
    }
