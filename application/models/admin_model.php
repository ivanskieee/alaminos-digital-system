...
class admin_model extends CI_Model
{
    ...
    public function saveResetDate($reset_date)
    {
        // Save the reset date in a table or configuration
        $this->db->set('reset_date', $reset_date);
        $this->db->where('id', 1); // Assuming only one row, you can adjust according to your table
        return $this->db->update('settings'); // Save in the settings table
    }

    public function getResetDate()
    {
        $query = $this->db->get_where('settings', ['id' => 1]); // Adjust the table and condition as needed
        return $query->row()->reset_date;
    }

    public function checkResetPoints()
    {
        $today = date('Y-m-d'); // Get today's date in Y-m-d format
        $reset_date = $this->admin_model->getResetDate(); // Get the reset date

        // Ensure that the reset date is in 'Y-m-d' format for comparison
        if ($reset_date && $today >= $reset_date) {
            // Reset points for all users
            $this->db->set('points', 0);
            $this->db->update('users');

            // Add a notification about the reset
            $this->admin_model->addNotification('Yearly points reset to 0.');

            // Update the reset date to the next year, same month, and day
            $next_year = date('Y', strtotime('+1 year'));
            $this->admin_model->saveResetDate($next_year . '-01-01');

        }
    }


    public function insertAuditLog($admin_id, $action)
    {
        $data = [
            'admin_id' => $admin_id,
            'action' => $action,
    ...
    {
        return $this->db->select('username')->where('admin_id', $admin_id)->get('admin')->row();
    }


    public function getAuditLogs()
    {
        $this->db->select('audit_logs.*, admin.username');
        $this->db->join('admin', 'audit_logs.admin_id = admin.admin_id');
        $this->db->order_by('audit_logs.timestamp', 'DESC');
        return $this->db->get('audit_logs')->result_array();
    }

    public function getUnreadAuditLogsCount()
    {
    ...
    {
        // Update all notifications to read (set is_read = 1)
        $this->db->set('is_read', 1);
        $this->db->update('audit_logs');
    }
    public function getUnreadNotificationCount()
    {
        $this->db->where('status', 'unread'); // Filter for unread notifications
        $this->db->from('audit_logs');
        return $this->db->count_all_results(); // Get count of unread logs
    }

    // Method to mark all notifications as read
    public function markAllNotificationsAsRead()
    {
        $this->db->set('status', 'read'); // Set status to 'read'
        $this->db->where('status', 'unread'); // Only update unread notifications
        $this->db->update('audit_logs'); // Update the status field
    }
